          ...
          <div className="lg:col-span-2 space-y-6">
              ...
              <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 space-y-4">
                <h3 className="font-bold text-slate-800 border-b border-slate-100 pb-2">
                  Approved Lifting Plan {formData.type === PermitType.LIFTING && <span className="text-red-500">*</span>}
                </h3>
                <div>
                  <label className="block text-sm font-semibold text-slate-700 mb-1">Select Approved Lifting Plan</label>
                  <select
                    ...
                    className="w-full border border-slate-300 rounded-lg p-2.5 text-sm"
                  >
                    <option value="">— Select approved lifting plan —</option>
                    {approvedLiftingPlans.map(plan => (
                      <option key={plan.id} value={plan.id}>
                        {plan.planNumber ? `${plan.planNumber} · ` : ''}{plan.title} — {plan.plan.equipmentType}
                      </option>
                    ))}
                  </select>
                  {approvedLiftingPlans.length === 0 && (
                    <div className="mt-2 bg-amber-50 border border-amber-200 rounded-lg p-3 text-sm text-amber-800">
                      ...
                      <Link to="/lifting-plans/new" className="inline-flex items-center mt-2 text-xs font-semibold text-blue-700 hover:underline">
                        Create Lifting Plan →
                      </Link>
                    </div>
                  )}
                ...
                {selectedLiftingPlan && (
                  <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 space-y-3">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        {selectedLiftingPlan.planNumber && (
                          <p className="text-xs font-mono text-blue-500 mb-0.5">{selectedLiftingPlan.planNumber}</p>
                        )}
                        <p className="font-bold text-blue-900 text-base">{selectedLiftingPlan.title}</p>
                        {selectedLiftingPlan.project && (
                          <p className="text-xs text-blue-700 mt-0.5">Project: {selectedLiftingPlan.project}</p>
                        )}
                      </div>
                      <Link
                        to={`/lifting-plans/${selectedLiftingPlan.id}`}
                        className="flex-shrink-0 flex items-center gap-1 text-xs font-bold text-blue-700 bg-white border border-blue-300 px-3 py-1.5 rounded-lg hover:bg-blue-50 transition-colors whitespace-nowrap"
                      >
                        View Plan →
                      </Link>
                    </div>
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 pt-1 border-t border-blue-200">
                      {[
                        { label: 'Equipment', value: selectedLiftingPlan.plan.equipmentType },
                        { label: 'Category', value: selectedLiftingPlan.plan.liftCategory || '—' },
                        { label: 'Load Weight', value: selectedLiftingPlan.plan.loadWeight !== null ? `${selectedLiftingPlan.plan.loadWeight} t` : '—' },
                        {
                          label: 'Utilization',
                          value: selectedLiftingPlan.plan.calculation
                            ? `${selectedLiftingPlan.plan.calculation.utilizationPercent.toFixed(1)}%`
                            : '—',
                        },
                        { label: 'Author', value: selectedLiftingPlan.author },
                        {
                          label: 'Approved',
                          value: selectedLiftingPlan.plan.approvedAt
                            ? new Date(selectedLiftingPlan.plan.approvedAt).toLocaleDateString()
                            : '—',
                        },
                      ].map(f => (
                        <div key={f.label}>
                          <p className="text-[10px] uppercase font-bold text-blue-600">{f.label}</p>
                          <p className="text-sm font-semibold text-blue-900">{f.value}</p>
                        </div>
                      ))}
                    </div>
                    {selectedLiftingPlan.plan.hseApprover && (
                      <p className="text-xs text-blue-700 flex items-center gap-1">
                        <CheckCircle size={12} /> Approved by {selectedLiftingPlan.plan.hseApprover}
                      </p>
                    )}
                    <div className="text-xs text-blue-600 bg-blue-100 rounded-lg px-3 py-2">
                      Attached to Permit · Plan locked in read-only mode
                    </div>
                  </div>
                )}
                {formData.type !== PermitType.LIFTING && (
                  <p className="text-xs text-slate-500">Only required for Lifting Permit type.</p>
                )}
              </div>
